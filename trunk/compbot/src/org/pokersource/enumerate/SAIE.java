...
public class SAIE {
  ...
  */
  public static void FlopGameSAIE(int gameType,
	...
	if (nmatchups == 0) {
      enumeration = new NestedLoopEnumeration(nhands);
    } else {
      enumeration = new NestedLoopSampling(nhands, nmatchups);
    }
  mainloop:
    while (enumeration.hasMoreElements()) { // loop over all hand matchups
      int[] indices = (int[]) enumeration.nextElement();
